...
<body>
	<main class="app">
		...
		<section class="layout">
			<article class="panel editor-panel">
				...
				<div class="editor-wrap"><div class="line-no" id="lineNo">1</div><textarea id="code" spellcheck="false" aria-label="Bytebeat expression">t*(1+(t&gt;&gt;10&amp;7)/16)*[1,1.25,1.5,2][t&gt;&gt;13&amp;3] + sin(t/200*(1+(t&gt;&gt;12&amp;3))) * 80 + sin(t/37)*30 + ((t*7&amp;t&gt;&gt;8)^(t*3&amp;t&gt;&gt;10))</textarea></div>
	...
	<script>
		...
		const downloadWav = () => { if (!compile()) return; const sampleRate = Number($('sampleRate').value), startSample = Math.floor(sampleT), beatSamples = Math.round(sampleRate * 60 / Number($('bpm').value)), output = new Float32Array(beatSamples * Math.min(10, Number($('maxBeats').value))); for (let i = 0; i < output.length; i++) output[i] = valueAt(startSample + i); const link = document.createElement('a'); link.href = URL.createObjectURL(makeWav(output, sampleRate)); link.download = 'bytebeat-' + startSample + '-' + output.length + 'samples.wav'; link.click(); setTimeout(() => URL.revokeObjectURL(link.href), 1000); $('status').textContent = 'downloaded ' + (output.length / sampleRate).toFixed(2) + 's WAV'; };
